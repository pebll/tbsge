...
func _test_move_cost() -> bool:
	var legion := Legion.new("GOBLIN", 2, Vector2i.ZERO, "GREEN")
...
func _test_swap_costs_both() -> bool:
	var a := Legion.new("GOBLIN", 2, Vector2i.ZERO, "GREEN")
	var b := Legion.new("GOBLIN", 2, Vector2i.ONE, "GREEN")
	a.spend_ap(1)
	b.spend_ap(1)
...
func _test_ap_from_unit_definition() -> bool:
	var rider := Legion.new("SCORPION_RIDER", 1, Vector2i.ZERO, "GREEN")
	if rider.max_ap != 3 or rider.current_ap != 3:
		push_error("SCORPION_RIDER should start with 3 AP from unit definition")
		return false
	var goblin := Legion.new("GOBLIN", 1, Vector2i.ZERO, "GREEN")
	if goblin.max_ap != 2:
		push_error("GOBLIN should start with default 2 AP")
		return false
	return true
