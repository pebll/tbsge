...
## Usage: ./run_ai_duel.sh [--games N] [--map-size 3|4|5] [--budget N] [--verbose] [--out-dir PATH]
...
func _initialize() -> void:
	...
	var batch: Dictionary = runner.run_batch(games, map_size, budget, verbose, out_dir)
...
func _print_help() -> void:
	...
	print("")
	print("Usage:")
	print("  ./run_ai_duel.sh [--games N] [--map-size 3|4|5] [--budget N] [--verbose] [--out-dir PATH]")
	print("")
	print("Options:")
	print("  --games N      Number of games to run (default: %d)" % DEFAULT_GAMES)
	print("  --map-size N   Hex map radius (default: %d)" % DEFAULT_MAP_SIZE)
	print("  --budget N     Gold per side (default: %d)" % DEFAULT_BUDGET)
	print("  --verbose      Print per-game results")
