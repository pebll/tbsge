...
func _test_single_battle_completes() -> bool:
	var result: Dictionary = BalanceBattleRunner.run("GOBLIN", "GOLEM", 42, 99)
	...
	if result.get("count_b", 0) != 6:
		push_error("Expected 6 golems for 60 gold, got %d" % result.get("count_b", 0))
		return false
	...
	var swapped: Dictionary = BalanceBattleRunner.run("GOBLIN", "GOLEM", 42, 100, false)
...
func _test_multi_legion_planning() -> bool:
	var goblin_sizes := BalanceArmyPlanner.plan_legion_sizes("GOBLIN", 13, 60, 12.0)
	if goblin_sizes.size() != 2:
		push_error("GOBLIN should use 2 maxed legions, got %s" % str(goblin_sizes))
		return false
	if goblin_sizes[0] != 12 or goblin_sizes[1] != 8:
		push_error("GOBLIN legions should be 12+8 at 60 gold, got %s" % str(goblin_sizes))
		return false
