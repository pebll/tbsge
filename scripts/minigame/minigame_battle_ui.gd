...
func _init(p_root: MinigameRoot) -> void:
	...
	interaction.battle_state_fn = func():
		if not is_instance_valid(root) or root.session == null:
			return null
		return root.session.battle_state()
	interaction.tile_visu_fn = func(coords: Vector2i) -> TileVisu:
		if not is_instance_valid(root):
			return null
		return root.presenter.tile_visu_at(coords)
	interaction.is_locked_fn = func() -> bool:
		if not is_instance_valid(root):
			return true
		return root.input_locked or root._ai_running
	interaction.can_act_fn = func(legion: Legion) -> bool:
		if not is_instance_valid(root) or root.session == null:
			return false
		return root.session.can_act_legion(legion)
	interaction.battle_phase_fn = func() -> bool:
		if not is_instance_valid(root) or root.session == null:
			return false
		return root.session.phase == MinigameSession.Phase.BATTLE
	...
	interaction.turn_manager_fn = func() -> TurnManager:
		if not is_instance_valid(root) or root.session == null:
			return null
		return root.session.turn_manager
	interaction.legions_fn = func() -> Array:
		if not is_instance_valid(root) or root.session == null:
			return []
		return root.session._typed_legions()
	interaction._inspect_fn = func(coords: Vector2i) -> void:
		if is_instance_valid(root):
			root.inspect_tile(coords)
	interaction._clear_inspect_fn = func() -> void:
		if is_instance_valid(root):
			root.clear_inspect()
