...
const DEFAULT_ACTION_IDS: Array[String] = ["move", "melee_attack", "ranged_attack", "self_heal"]
...
func legion_action_ids(legion: Legion) -> Array[String]:
	...
	var ids := default_action_ids()
	# Drop ranged from the default list when this unit type cannot shoot.
	if unit_def == null or not unit_def.has_ranged():
		ids.erase("ranged_attack")
	return ids
