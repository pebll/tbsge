...
func _ready() -> void:
	unit_type = UNITS[randi() % 8]
	unit_count = 4#randi() % 8 +1
	direction_front = true
	direction_right = true if randi() % 2 == 0 else false
	for i in unit_count:
		var unit = preload("res://scenes/legion_unit.tscn").instantiate()
...
func update_local_positions():
	...
	if unit_count == 4:
		...
		cols = ceil(sqrt(unit_count * 1.5))  # slightly wider than tall
	...
	for i in range(unit_count):
		...
		var y = (row - ceil(unit_count / cols) / 2.0 + 0.5) * base_spacing * 0.8
